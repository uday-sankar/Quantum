...
def run_mult(range_eps,Ra):
    ...
    for eps in range_eps:
        ...
        if len(range_eps)>50:
            sys.stdout.flush()
            sys.stdout.write("\r{0}>>".format('#'*int((i+1)/len(range_eps))))
        i+=1
    return data
...
def Energy_finder(E_est,num_prec=5,peak=[-1,1],Ra=[-5,5],div=.1):
    ...
    for i in range(num_prec):
        ...
        sys.stdout.flush()
        sys.stdout.write("\r{0}".format(f'\tEnergy search range after {i+1} runs {E_array[0]}->{E_array[-1]}'))
    return Eg_ar
...
print(tme)
i=0
for i in range(len(EGR)):
    plt.plot(D[i][0],D[i][1]+EGR[i],label=EGR[i])
plt.legend()
...
#optional program to see the values of explotion factor vs energies
'''Xplo_1=Xplo_e
ex_coef=[]
e=[]
for ens in Xplo_1:
    e.append(ens[1])
    ex_coef.append(ens[0])
plt.scatter(e,ex_coef)
